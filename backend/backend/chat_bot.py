    ...
    #try:
        ...
        if message.text in ("приветики","привет",
            ...
            def getCode_finish(message):
                try:
                    #CLient.objects.get(code=message.text) Здесь ты должен пробить код по твоей БД, подкоректируй этот момент
            ...
            try:
                ...
                #Короче, клавиатуры- это квест не на один день, а без них не очень понятно, что к чему цеплять
                #Вообщем, здесь сам сделай вывод боту твоих данных, просто привяжи к переменным данные из БД
                #А я, когда разберусь с клавиатурами, оформлю всё как надо
            except Client.DoesNotExist:
